...
import {validateUpdateUser} from "../utils/validation/userValidation.js";
...
export const followOrUnfollowUser = asyncHandler(async (req, res) => {
    ...
    if (id === req.user._id.toString()) {
        return res.status(400).json({ status: STATUS.FAIL, message: "You cannot follow/unfollow yourself" });
    }

    if (!userToModify || !currentUser) return res.status(404).json({ status: STATUS.FAIL, message: "User not found" });

    const isFollowing = currentUser.following.includes(id);

    if (isFollowing) {
        // Unfollow user
        await User.findByIdAndUpdate(id , {$pull: { followers: currentUser._id }}, { new: true });
        await User.findByIdAndUpdate(currentUser._id, {$pull: { following: id }}, { new: true });
        ...
        // Follow user
        await User.findByIdAndUpdate(id , {$push: { followers: currentUser._id }}, { new: true });
        await User.findByIdAndUpdate(currentUser._id, {$push: { following: id }}, { new: true });
...
export const getSuggestedUsers = asyncHandler(async (req, res) => {
    ...
    // Take first 4 only
    const filteredSuggestions = suggestedUsers.filter(suggestedUser => !followingList.includes(suggestedUser._id));
...
export const updateUserProfile = asyncHandler(async (req, res) => {
    ...
    res.status(200).json({ status: STATUS.SUCCESS, message: "Profile updated successfully", data: userUpdate });
